...
public class PersistenceManager {
	...
	private boolean writeToLog(int transactionID, PageData page) {
		try {
			...
			File logFile = new File(String.format(userDir + "/log/log.dat",
					page.getLogSequenceNumber()));
			FileWriter fw = new FileWriter(logFile, true);
			fw.write(String.format("%d,%d,%d,%s%s",
					page.getLogSequenceNumber(), transactionID,
	...
	private boolean writeToDisk(PageData page) {
		try {
			...
			File file = new File(String.format(userDir + "/data/%s.dat",
					page.getPageID()));
	...
	public synchronized void commit(int transactionID) {
		...
		try {
			...
			fw.write(String.format("%d,%d,commit%s", lsn, transactionID,
					System.getProperty("line.separator")));
			fw.close();
		} catch (Throwable e) {
			System.err.println("Logfile could not be written: "
					+ e.getMessage());
		}
	}
